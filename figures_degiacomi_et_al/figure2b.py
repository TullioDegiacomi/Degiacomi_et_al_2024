...
data = pd.read_csv('Parameters_CTRL_sounding.txt',delimiter = '\t')
data2 = pd.read_csv('sounding_all_variables.txt',delim_whitespace=True)
...
ax1.set_xlabel("Wind Speed [m s$^{-1}$]",color='gray')
...
#ax3.set_xlabel('$\mathrm{\Theta}$, $\mathrm{\Theta_e}$ [K]', color="green",labelpad=20) 
...
ax3.text(266, 8600, '(b)', color='black', ha='center',fontsize=36)
...
ax3.text(305.2, 2500, '$N^{2}_{3}$ = 0.00008 s$^{-2}$',  fontsize=24,
          bbox={'facecolor': 'grey', 'alpha': 0.5})

ax3.text(302., 1400, '$N^{2}_{2}$ = 0.00019 s$^{-2}$',  fontsize=24,
          bbox={'facecolor': 'grey', 'alpha': 0.5})

ax3.text(298, 500, '$N^{2}_{1}$ = 0.00008 s$^{-2}$',  fontsize=24,
          bbox={'facecolor': 'grey', 'alpha': 0.5})
